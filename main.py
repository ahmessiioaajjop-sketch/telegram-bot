...
import json
API_ID = '38819452'
API_HASH = '3850e0caf397a3119c2dbcbd335e025e'
bot_token = "8533912487:AAEnv9hlsUQn9BwQvQp782pRN7doxKgCJRY"
running_processes = {}
...
async def background_task(phonex, bot_username, sudo, send_to):
    ...
    clients[f"{phonex}-{sudo}"] = TelegramClient(f"echo_ac/{sudo}/{phonex}", API_ID, API_HASH, device_model="Devloper : @GJEGG1")
    ...
    if update.message and update.message.chat.type == "private":
        if (str(update.message.chat.id) == str(info["sudo"])):
            ...
            keyboard = [
                ...
                [
                    InlineKeyboardButton(
                        "قناة المطور ⚜️", url="t.me/GJEGG1"),
                ],
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            await update.message.reply_text("""مرحبآ بك في بوت المخضرم.البوت مخصص بتجميع نقاط جميع البوتات وبخدمات ومنيزات رائعه داخل البوت\n\nDev : @GJEGG1""", reply_markup=reply_markup)
...
async def joinchn(id, chn):
    ...
    for file in file_list:
        ...
        async with TelegramClient(f"echo_ac/{id}/{file}", API_ID, API_HASH, device_model="Devloper : @GJEGG1") as client:
            try:
    ...
    if update.message.text and update.message.text.startswith("/run "):
        ...
        if (what_need_to_do_echo[str(update.message.chat.id)] == "addecho"):
            ...
            client = TelegramClient(
                f"echo_ac/{update.message.chat.id}/{update.message.text}", API_ID, API_HASH, device_model="Devloper : @GJEGG1")
            try:
                await client.connect()
            ...
            client = TelegramClient(f"echo_ac/{update.message.chat.id}/"+str(
                what_need_to_do_echo[str(update.message.chat.id)+":phone"]), API_ID, API_HASH, device_model="Devloper : @GJEGG1")
            await client.connect()
            try:
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "addecho":
        ...
        for file in file_list:
            ...
            file = file.replace('.session','')
            client = TelegramClient(
                f"echo_ac/{id}/{file}", API_ID, API_HASH, device_model="Devloper : @GJEGG1"
            )
            try:
                await client.connect()
                if not await client.is_user_authorized():
...
#hello
        ...
        if (str(query.message.chat.id) == str(info["sudo"])):
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text("مرحبآ بك في بوت المخضرم.البوت مخصص بتجميع نقاط جميع البوتات وبخدمات ومنيزات رائعه داخل البوت\n\nDev : @GJEGG1", reply_markup=reply_markup)
        elif (str(query.message.chat.id) in info["admins"]):
            keyboard = [
                [
                    InlineKeyboardButton(
        ...
        try:
            client = TelegramClient(
                f"echo_ac/{query.message.chat.id}/{filename}", API_ID, API_HASH, device_model="Devloper : @GJEGG1")
            await client.connect()
            await client.log_out()
            await client.disconnect()
            what_need_to_do_echo[str(query.message.chat.id)] = ""
